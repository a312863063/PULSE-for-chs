...
parser.add_argument('-synthesis_weight', type=str, default='cache/synthesis.pt', help='directory for synthesis model weights')
parser.add_argument('-mapping_weight', type=str, default='cache/mapping.pt', help='directory for mapping model weights')
...
parser.add_argument('-steps', type=int, default=1000, help='Number of maximum optimization steps')
parser.add_argument('-lr_schedule', type=str, default='linear1cycledrop', help='fixed, linear1cycledrop, linear1cycle')
parser.add_argument('-save_intermediate', default=False, action='store_true', help='Whether to store and save intermediate HR and LR images during optimization')
...
model = PULSE(synthesis_weight=kwargs["synthesis_weight"], mapping_weight=kwargs["mapping_weight"])
...
for ref_im, ref_im_name in dataloader:
    if(kwargs["save_intermediate"]):
        ...
        for j,(HR,LR) in enumerate(model(ref_im, **kwargs)):
            for i in range(kwargs["batch_size"]):
                toPIL(HR[i].cpu().detach().clamp(0, 1)).save(
                    out_path / f"{ref_im_name[i]}_output.png")
